...
function initMechantShader() {
    splatShader = initShaders("mechant-vs","mechant-fs");

    // active ce shader
    gl.useProgram(splatShader);

    // adresse des variables uniform dans le shader
    splatShader.positionUniform = gl.getUniformLocation(splatShader, "uMechantPosition");
    splatShader.texUniform = gl.getUniformLocation(splatShader, "uMechantTex");
    splatShader.couleurUniform = gl.getUniformLocation(splatShader, "maCouleurMechant");

    console.log("splat mechant initialized");
}

function MechantPokemon(splatTexture) {
...
MechantPokemon.prototype.shader = function() {
    return splatShader;
}

MechantPokemon.prototype.initParameters = function() {
...
MechantPokemon.prototype.setPosition = function(x,y,z) {
    this.position = [x,y,z];
}
...
MechantPokemon.prototype.draw = function() {
    // dessin du splat
    if(this.loaded) {
	gl.bindVertexArray(this.vao);
...
MechantPokemon.prototype.clear = function() {
